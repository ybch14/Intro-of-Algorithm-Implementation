...
BST::~BST()
{
	...
	std::stack<BSTNode*> s;
	std::vector<BSTNode*> delete_buffer;
	if (_root != NULL)
	{
		s.push(_root);
		BSTNode *cur = _root, *tmp = NULL;
...
BSTNode* BST::search(double value)
{
	BSTNode* cur = _root;
...
BSTNode* BST::successor(BSTNode* cur)
{
	if (cur == NULL)
		return NULL;
...
BSTNode* BST::predecessor(BSTNode* cur)
{
	if (cur == NULL)
		return NULL;
...
void BST::insert(double value)
{
	...
	else
	{
		...
		if (value < cur->value())
		{
			cur->_left = new BSTNode(value);
			cur->_left->_parent = cur;
		}
		else
		{
			cur->_right = new BSTNode(value);
			cur->_right->_parent = cur;
		}
	}
}

void BST::remove(BSTNode* dst)
{
	if (_root == NULL)
	{
	...
	else
	{
		...
		{
			BSTNode* suc = this->successor(dst); // no left child
			if (suc->_right != NULL)
			{
...
void printTree(BSTNode* root, int space, int max_width)
{
	if (root == NULL)
		return;
